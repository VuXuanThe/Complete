...
public class DataConnectForTeacherFragment extends Fragment implements IHandlerAndLoadNotice{
    public final static String TAG = "DataConnectForTeacherFragment";
    private FragmentDataConnectForTeacherBinding binding;
    private DataConnect dataConnect;
    private DataConnectForTeacherPresenter presenter;
    private APIService apiService;

    public static DataConnectForTeacherFragment newInstance(Bundle bundle) {
        DataConnectForTeacherFragment fragment = new DataConnectForTeacherFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        binding = DataBindingUtil.inflate(inflater, R.layout.fragment_data_connect_for_teacher, container, false);
    ...
    private void setView(){
        ...
        if(dataConnect.getUpdate().equals("Wait Teacher")){
            binding.tvNoticeConfirmUpdate.setText(Helper.setUnderlineString("Thông báo: Xác nhận cập nhật thông tin từ " + dataConnect.getStudent().getStudentName()));
            ...
            public void onClick(View v) {
                ...
                builder.setPositiveButton("Xác nhận", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        //create notice
                        Notice notice = new Notice("Xác nhận cập nhật", String.valueOf(System.currentTimeMillis()), dataConnect.getTeacher().getTeacherName() + " đã xác nhận cập nhật!");
                        createNotice(notice);

                        //create notification
                        Data data = new Data(FirebaseAuth.getInstance().getCurrentUser().getUid(), Helper.TEACHER_TV + " " + dataConnect.getTeacher().getTeacherName() + " đã xác nhận môn học bạn cạp nhật", "Confirm Request",
                ...
                builder.setMessage("Xác nhận cập nhật từ Học sinh " + dataConnect.getStudent().getStudentName() + ", Nếu không đúng thông tin, hay liên hệ với học sinh qua phần tin nhắn!");
        ...
        binding.btnAddOneNumsessions.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(!dataConnect.getUpdate().equals("Updated")){
                    ...
                    Notice notice = new Notice("Tăng buổi học", String.valueOf(System.currentTimeMillis()), dataConnect.getTeacher().getTeacherName() + " đã tăng lên một buổi học!");
                    createNotice(notice);

                    //create notification
                    Data data = new Data(FirebaseAuth.getInstance().getCurrentUser().getUid(), Helper.TEACHER_TV + " " + dataConnect.getTeacher().getTeacherName() + " đã tăng lên một buổi học", "Tăng buổi học",
                            dataConnect.getUidStudent());
                    sendNotificationRequest(data);
                }
            }
        ...
        binding.btnDeviceNumSessions.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(!dataConnect.getUpdate().equals("Updated")){
                    ...
                    //push notification
                    Data data = new Data(FirebaseAuth.getInstance().getCurrentUser().getUid(), Helper.TEACHER_TV + " " + dataConnect.getTeacher().getTeacherName() + " đã giảm đi một buổi học", "Giảm buổi học",
                            dataConnect.getUidStudent());
                    sendNotificationRequest(data);

                    Notice notice = new Notice("Giảm buổi học", String.valueOf(System.currentTimeMillis()), dataConnect.getTeacher().getTeacherName() + " đã giảm đi một buổi học!");
                    createNotice(notice);
                }
            }
        });
        ...
        binding.btnConfirmTuition.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(newTuition.equals(dataConnect.getTuition())){
                    ...
                    Notice notice = new Notice("Học Phí", String.valueOf(System.currentTimeMillis()), dataConnect.getTeacher().getTeacherName() + " đã thay đổi học phí thành " + newTuition);
                    createNotice(notice);

                    //push notification
                    Data data = new Data(FirebaseAuth.getInstance().getCurrentUser().getUid(), Helper.TEACHER_TV + " " + dataConnect.getTeacher().getTeacherName() + " đã thay đổi học phí", "Học phí",
                            dataConnect.getUidStudent());
                    sendNotificationRequest(data);
